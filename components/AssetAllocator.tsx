...
const AssetAllocator: React.FC<AssetAllocatorProps> = ({ candidate, assets, setAssets, readOnly, onLog }) => {
  ...
  const handleAllocate = (assetId: string) => {
    ...
    // FIX 1: Check if the ASSET is actually available
    if (assetToAssign.status !== AssetStatus.AVAILABLE) {
        addToast(`Error: Asset ${assetToAssign.name} is currently ${assetToAssign.status}`, 'error');
        return;
    }

    // FIX 2: Check if Candidate already has this TYPE of asset
